...
class CHolePhpGaclAdapter
{
	...
	function __construct()
	{
		...
		//$aclDb = Yii::app()->getComponent('holeIdentityDb');
		//var_dump($aclDb->getAttribute());die;
		
		$gacl_options['debug'] = '';
		$gacl_options['db_type'] = 'mysql' ;
		$gacl_options['db_host'] = '173.203.97.211';
		$gacl_options ['db_user'] = 'himawan';
		$gacl_options['db_password'] = 'putra';
		$gacl_options['db_name'] = 'newhol';
		...
		$this->_aclEngine = new gacl_api($gacl_options);
		
		//$this->_acl = new Kutu_Acl_Vendor_PhpGacl($gacl_options);
		$this->_acl = new gacl($gacl_options);
	}
	function getUsers()
	{
...
//		return $this->_aclEngine->del_group_object($aroGroupId, 'user', $username, $group_type='ARO');
	...
	{
		if ($this->checkAcl('action', 'execute', 'user', $username, 'admin', 'all-access'))
				return true;
